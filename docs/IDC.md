...
1. Choose **Load body-region filters**, then select a region, or leave **All regions** selected. Region labels come from IDC metadata across all modalities, so some have no MRI results. Scans without a body-region tag appear only under All regions.
...
3. Choose **Find MRI series**. Results show acquisition descriptions, participant and collection IDs, DICOM instance counts, and estimated download sizes. Study and series identifiers are expandable.
...
- Ten series are requested per page. Previous/next pages require an explicit action; there is no background crawl.
...
This connector loads one series at a time. Bulk participant collections, IDC segmentations, clinical-table queries, free-text clinical search and pinned historical IDC releases are not implemented. OpenNeuro and Zenodo remain available in their existing workflows; fastMRI is not connected.
...
Tests use synthetic metadata and mocked API requests to check MRI filtering, query bounds, cache reuse, cancellation, storage allowlisting, size limits and complete-series validation. They make no requests to public repositories.
