...
Catalog archive checks have a ten-second per-record deadline. An empty compatible-results page retains the next-page cursor for the operator to continue explicitly. Slow or failed checks are reported as unchecked, not incompatible. This deadline only applies to discovery; explicitly opening a record or downloading a scan retains the normal import deadline. Live browser search for `knee` returned the bilateral qDESS knee dataset (7749765).
