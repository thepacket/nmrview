...
NMRView uses NiiVue, jcampconverter, React, and other packages recorded in `package-lock.json`. Each dependency retains its own license and copyright notices. Consult the installed package's license when redistributing it; this document is not an exhaustive transitive dependency license inventory.

Online datasets are governed by the license shown by their source repository. Availability for free download does not mean that every reuse is unrestricted.
