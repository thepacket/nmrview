...
import { describeRepositoryFile } from "@/lib/repository-file-guide";
...
export function StudyComparison({
  ...
  const [selected, setSelected] = useState<string[]>(
    collection.studies.slice(0, 2).map((s) => s.id),
  );
...
function ComparisonPane({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const image = await NVImage.loadFromFile({ file: scan });
        if (abort.signal.aborted) return;
  ...
  return (
    <article className="comparison-pane">
      ...
      <Choice
        label={`Scan for ${study.id}`}
        value={file.name}
        options={study.files.map((f) => [
          f.name,
          `${describeRepositoryFile(f.name, "mri").label} · ${f.name.split("/").at(-1)}`,
        ])}
        onChange={onFile}
      />
      <div className="comparison-canvas">
        <canvas ref={canvas} aria-label={`MRI comparison ${study.id}`} />
